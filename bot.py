...
# ── ১. Telegram ─────────────────────────────────────────────────────
BOT_TOKEN_VALUE          = "8727550482:AAEdUJt5wyHum5CHrcH1CnyT8Y-i-GOWlRE"
...
async def create_product(name: str, price: float, emoji: str = "📮", category: str = "mail",
                         delivery_mode: str = "manual", hidden: bool = False) -> str:
    return await db_push("products", {
        "name": name, "price": price, "emoji": emoji, "category": category,
        "delivery_mode": delivery_mode, "hidden": hidden,
        ...
        try:
            ...
            with _urllib_request.urlopen(req, timeout=10) as res:
                ...
                if isinstance(data, list):
                    return [d.get("name", d) if isinstance(d, dict) else str(d) for d in data]
                if isinstance(data, dict):
                    return list(data.get("domains", data.keys()))
        except Exception as e:
...
async def mail_qty_entered(message: Message, state: FSMContext):
    ...
    if err:
        ...
        return
    pid     = data["pid"]
    product = await get_product(pid) or data["product"]
...
async def _vpn_confirm(message: Message, state: FSMContext, days: int):
    data    = await state.get_data()
    pid     = data["vpn_pid"]
    product = await get_product(pid) or data["vpn_product"]
...
async def vpn_coupon_code_entered(message: Message, state: FSMContext):
    ...
    days    = data["vpn_days"]
    disc    = coupon["discount_pct"]
    price   = round(product["price"] * days * (1 - disc / 100), 4)
...
async def proxy_auto_qty_entered(message: Message, state: FSMContext):
    ...
    if err:
        ...
        return
    pid     = data["proxy_pid"]
    product = await get_product(pid) or data["proxy_product"]
...
async def proxy_custom_days(message: Message, state: FSMContext):
    if message.text in (CANCEL_BTN, BACK_BTN):
        ...
        data_opts = data.get("proxy_data_opts", "1 GB,5 GB,10 GB,50 GB")
        await state.set_state(UserFlow.proxy_duration)
        await message.answer(f"🔐 <b>{prod.get('name','')}</b>\n{_SEP}\nSelect data amount:", reply_markup=proxy_data_kb(data_opts))
        return
...
async def _proxy_confirm(message: Message, state: FSMContext, data_label: str):
    ...
    price   = round(product["price"] * amount, 4)
...
async def proxy_coupon_code_entered(message: Message, state: FSMContext):
    ...
    amt     = amt or 1.0
    price   = round(product["price"] * amt * (1 - disc / 100), 4)
...
async def proxy_confirm(message: Message, state: FSMContext):
    ...
    if data.get("proxy_coupon_id"):
        await use_coupon(data["proxy_coupon_id"])
    oid = await create_proxy_order(uid, user.get("username",""), data["proxy_pid"], product["name"], days, price)
    await state.clear()
    await message.answer(
...
async def deposit_amount_entered(message: Message, state: FSMContext):
    ...
    if is_usd:
        ...
        if amount_usd <= 0:
            ...
            return
        if amount_usd < data["min_amt"]:
            await message.answer(f"❌ Minimum Binance deposit is <b>${data['min_amt']:.2f} USD</b>")
            return
    ...
    await message.answer(fmt_deposit_confirm(data["method_label"], amount_bdt, amount_usd, is_usd), reply_markup=input_kb())
...
async def deposit_trx_entered(message: Message, state: FSMContext):
    if message.text in (CANCEL_BTN, BACK_BTN):
        data = await state.get_data()
        await state.set_state(UserFlow.dep_amount)
        await message.answer(fmt_deposit_info(data["method_label"], "", data["rate"], data["min_amt"], data.get("is_usd", False)), reply_markup=input_kb())
        return
...
async def deposit_screenshot(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        fmt_deposit_submitted(dep_id, data["method_label"], data["amount_bdt"], data["amount_usd"], data.get("is_usd", False)),
        reply_markup=main_menu_kb(),
    )
    for adm in ADMIN_IDS:
        try:
